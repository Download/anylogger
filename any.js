...
/**
 * anylogger([name] [, options]) => function logger([level='debug'] [, ...args])
 * 
 ...
 * @return function log([level='debug'], args...)
 */
a.new = function(n,r) {
 ...
 * 
 * `anylogger.log([level='debug',] ...args)`
 * 
 ...
 * log at (defaults to 'debug') and then calls the correct method on the 
 * logger function with the remaining arguments. 
 */
a.log = function(n,x) {
  m[n][x.length > 1 && a.levels[x[0]] ? x.shift() : 'debug'].apply(m[n], x)
}
